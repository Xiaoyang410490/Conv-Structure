...
#preprocess the image,in order to get better results
X_train_rgb = X_train
X_train_gry = np.sum(X_train / 3, axis=3, keepdims=True) #keepdims = True, so that the dimension changes it not
X_test_rgb = X_test
X_test_gry = np.sum(X_test / 3, axis=3, keepdims=True)
# gry image here will be used
X_train_normalized = (X_train_gry - 128.) / 128.
X_test_normalized = (X_test_gry - 128.) / 128.
# normalization,to get gauss distribution

def weight_variable(shape):
    initial = tf.truncated_normal(shape, mean=0.0, stddev=0.1)
    #standard deviation can also be set as hyperprameter
...
#we use different max_pool layers
def max_pool_2x2(x):
...
#generate the batch,the network will be trained with vatious batch of data
...
#use conv layer and max-pool layer to get information from the image
W_conv1 = weight_variable([3, 3, 1, 6])
b_conv1 = bias_variable([6])
h_conv1 = tf.nn.elu(conv2d(x, W_conv1) + b_conv1)

h_mul1 = max_pool_5x5(h_conv1)
W_mul1 = weight_variable([1,1,6,1])
b_mul1 = bias_variable([1])
h_mul1 = tf.nn.elu(conv2d(h_mul1,W_mul1)+b_mul1)
...
h_mul2 = max_pool_4x4(h_conv2)
W_mul2 = weight_variable([1,1,16,1])
b_mul2 = bias_variable([1])
h_mul2 = tf.nn.elu(conv2d(h_mul2,W_mul2)+b_mul2)
...
h_mul3 = max_pool_2x2(h_conv3)
W_mul3 = weight_variable([1,1,16,1])
b_mul3 = bias_variable([1])
h_mul3 = tf.nn.elu(conv2d(h_mul3,W_mul3)+b_mul3)
...
#concat these vector together
h_pool2_flat = tf.reshape(h_pool2, [-1, 144])
h_mul1_flat = tf.reshape(h_mul1,[-1,36])
h_mul2_flat = tf.reshape(h_mul2,[-1,9])
h_mul3_flat = tf.reshape(h_mul3,[-1,16])

h_pool2_flat = tf.concat([h_pool2_flat,h_mul1_flat],axis=1)
h_pool2_flat = tf.concat([h_pool2_flat,h_mul2_flat],axis=1)
h_pool2_flat = tf.concat([h_pool2_flat,h_mul3_flat],axis=1)

keep_prob = tf.placeholder("float32")
h_pool2_flat = tf.nn.dropout(h_pool2_flat,keep_prob)

W_fc2 = weight_variable([205, 108])
b_fc2 = bias_variable([108])
h_fc2 = tf.nn.elu(tf.matmul(h_pool2_flat, W_fc2) + b_fc2)
h_fc2_drop = tf.nn.dropout(h_fc2, keep_prob)

W_fc3 = weight_variable([108, 43])
