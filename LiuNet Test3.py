...
h_conv12 = tf.nn.leaky_relu((conv2d(h_conv1, W_conv12) + b_conv12),alpha=0.20)
h_pool1 = max_pool_2x2(h_conv12)
h_pool12 = max_pool_4x4(h_conv12)
...
h_conv22 = tf.nn.leaky_relu((conv2d(h_conv2, W_conv22) + b_conv22),alpha=0.20)
h_pool2 = max_pool_2x2(h_conv22)
h_pool21 = max_pool_5x5(h_conv22)
h_pool3 = max_pool_5x5(h_pool2)

h_pool12_flat = tf.reshape(h_pool12,[-1,294])
h_pool13_flat = tf.reshape(h_pool13,[-1,96])
h_pool2_flat = tf.reshape(h_pool2, [-1, 400])
h_pool21_flat = tf.reshape(h_pool21,[-1,64])
h_pool3_flat = tf.reshape(h_pool3,[-1,16])

h_pool2_multi = tf.concat([h_pool12_flat,h_pool13_flat],axis=1)
h_pool2_multi = tf.concat([h_pool2_multi,h_pool2_flat],axis=1)
h_pool2_multi = tf.concat([h_pool2_multi,h_pool21_flat],axis=1)
h_pool2_multi = tf.concat([h_pool2_multi,h_pool3_flat],axis=1)

W_fc1 = weight_variable([870, 400])
b_fc1 = bias_variable([400])
h_fc1 = tf.nn.leaky_relu((tf.matmul(h_pool2_multi, W_fc1) + b_fc1),alpha = 0.20)
keep_prob = tf.placeholder("float32")
h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)

W_fc2 = weight_variable([400, 120])
b_fc2 = bias_variable([120])
h_fc2 = tf.nn.leaky_relu((tf.matmul(h_fc1_drop, W_fc2) + b_fc2),alpha = 0.20)
h_fc2_drop = tf.nn.dropout(h_fc2, keep_prob)

W_fc3 = weight_variable([120, 84])
b_fc3 = bias_variable([84])
h_fc3 = tf.nn.leaky_relu((tf.matmul(h_fc2_drop, W_fc3) + b_fc3),alpha = 0.20)
h_fc3_drop = tf.nn.dropout(h_fc3, keep_prob)

W_fc4 = weight_variable([84, 43])
b_fc4 = bias_variable([43])
